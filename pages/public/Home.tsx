    ...
    <div className="min-h-screen bg-white">
      ...
      <nav className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 flex items-center justify-between">
        ...
        <div className="flex items-center space-x-4">
          <Link to="/login" className="text-gray-700 font-semibold hover:text-blue-600">Login</Link>
          <Link to="/signup" className="px-5 py-2.5 bg-blue-600 text-white font-semibold rounded-lg hover:bg-blue-700 transition-colors">Start Free</Link>
        </div>
      </nav>
      ...
      <section className="relative pt-20 pb-32 overflow-hidden">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex flex-col items-center text-center">
          ...
          <div className="flex flex-col sm:flex-row space-y-4 sm:space-y-0 sm:space-x-6">
            <Link to="/signup" className="px-8 py-4 bg-gray-900 text-white rounded-xl text-lg font-bold hover:bg-gray-800 transition-all shadow-xl shadow-gray-200">
              Get Started Now
            </Link>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            <FeatureCard 
              ...
              description="Real-time, low-latency natural voice conversations with domain-specific AI agents." 
            />
            <FeatureCard 
              icon={FileText} 
              title="JD + Resume Intelligence" 
              description="Automatically match candidates to Job Descriptions with deep skill extraction." 
            />
            <FeatureCard 
              icon={Microscope} 
              title="Automated AI Scoring" 
              description="Standardized scoring based on technical competency and cultural fit analysis." 
            />
            <FeatureCard 
              icon={Globe} 
              title="Multi-tenant SaaS" 
              description="Isolated environments for every organization with custom branding and agents." 
            />
            <FeatureCard 
              icon={ShieldCheck} 
              title="GDPR Compliant" 
              description="Full security suite with audit logs, data encryption, and bias detection monitors." 
            />
            <FeatureCard 
              icon={CheckCircle} 
              title="ATS Integration" 
              description="Seamlessly sync results back to your existing Applicant Tracking Systems." 
            />
          </div>
        </div>
      </section>
